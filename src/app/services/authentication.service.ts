...
export class AuthenticationService {
    ...
    // environment clone authLogic
    environmentAuthLogic(
        userName: string,
        password: string,
        ...
        return this.http.post<AppUserAuth>(body, options).pipe(
            map((response: any) => {
        ...
        return this.http.post<AppUserAuth>(url, body, options).pipe(
            map((response: any) => {
                ...
                if (`${tokenData.exp}`.length > 10) {
                    console.warn(
                        `expiration in token has more than 10 chars, it may be milliseconds when we expect seconds`
                    );
                }
                const expiresAtMs = tokenData.exp * 1000;
                this.persistsAuthData(
                    data.accessToken,
                    expiresAtMs,
                    tokenData,
                    tokenData.servicerId
                );
    ...
    logout(continueToAfterLogin: string = ''): void {
        this.clearAuthData();
        this.authenticated.next(false);
        this.dialog.closeAll();
        if (continueToAfterLogin) {
            this.router.navigate(['/login'], {
                queryParams: { continue: continueToAfterLogin }
            });
        } else {
            this.router.navigate(['/login']);
        }
    ...
    public securityObjectRolesIncludes(role: AuthRoleEnum): boolean {
        ...
        return (
            securityObject &&
            securityObject['cognito:groups'] &&
            securityObject['cognito:groups'].includes(role)
        );
    }
